...
// 获取着色器文本 (glsl es语言) 
const vsSource = document.querySelector('#drawTriangleVertexShader').textContent
const fsSource = document.querySelector('#drawTriangleFragmentShader').textContent
...
const randerPoints = (gl) => {
    const counts = [[-0.085, -0.05], [0, 0.1], [0.085, -0.05], [0.17, 0.1]]
    // 记录顶点数据
    ...
    // 以下较为复杂
    // 对应 doc/绘制三角形-前半部分/文章/03-绘制三角形.md 409行开始看
    // 或对应第二章第4节： 绘制面视频看
    // 5.绘图(画连续三角带)
    gl.drawArrays(gl.TRIANGLE_STRIP, 0, counts.length)
    // 绘图(画连续三角扇)
    // gl.drawArrays(gl.TRIANGLE_FAN, 0, counts.length)
}
</script>
